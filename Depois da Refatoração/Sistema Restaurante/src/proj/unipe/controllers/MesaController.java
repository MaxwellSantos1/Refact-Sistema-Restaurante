...
public class MesaController {
	...
	/**
	 * @deprecated Use {@link #salvar(Mesa,BindingResult,ModelMap)} instead
	 */
	@RequestMapping(method = RequestMethod.POST, value = {"salvar"})
	public String save(@ModelAttribute("mesa") @Valid Mesa mesa, BindingResult result, ModelMap map) {
		return salvar(mesa, result, map);
	}

	@RequestMapping(method = RequestMethod.POST, value = {"salvar"})
	public String salvar(@ModelAttribute("mesa") @Valid Mesa mesa, BindingResult result, ModelMap map) {
		if(result.hasErrors()){
			map.addAttribute("mesa", mesa);
			map.addAttribute("selectDeReserva", selectDeReserva());
			return "mesa/novo";
